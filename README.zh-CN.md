...
- 配置字段：`Cookie`
- 当前状态：保留 Provider 结构与兼容代码，默认不在新增列表中开放
...
- 根 README 现在作为语言入口页，中文与英文说明分别维护
- 发布无关的 `output/playwright` 截图产物已从仓库移除
- 未被使用的 `src/renderer/src` 脚手架残留已从仓库移除
...
- 透明浮窗在极端桌面环境下仍建议做一次人工体验复核，重点关注边缘把手和透明度较低时的点击可用性。
