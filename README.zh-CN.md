...
![应用总览](docs/assets/readme/overview.png)
...
![设置面板](docs/assets/readme/settings-focus.png)

## 核心能力

- 桌面常驻浮窗，支持折叠态、展开态和边缘吸附（当前仅支持右侧吸附）
...
前往 [GitHub Releases](https://github.com/JJChou000/coding-plan-usage-tracker/releases) 下载 `v0.3.0` 或更新版本的 Windows 安装包。
发布资产请优先使用 `Coding.Plan.Usage.Tracker.Setup.0.3.0.exe`，不要上传或分发 `win-unpacked/` 目录中的开发产物。
...
正式发布时请以 NSIS 安装包 `Coding Plan Usage Tracker Setup 0.3.0.exe` 为准，体积应在约 80 MB。
...
6. 将浮窗拖拽到屏幕右边缘后会自动吸附，点击边缘把手可恢复显示。

## 吸附说明

- 当前版本仅支持拖拽到屏幕右边缘时吸附
- 拖拽到屏幕上、左、下边缘时不会进入吸附态
- 历史版本若曾保存上、左、下吸附状态，应用会在启动或托盘恢复时自动回退到可视区域
